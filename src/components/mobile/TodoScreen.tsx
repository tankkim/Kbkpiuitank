...
export function TodoScreen({ onBack }: TodoScreenProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <MobileHeader title="Daily To-Do List" onBack={onBack} />
      ...
      <div className="p-3">
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <div className="bg-white p-2 rounded-lg border border-gray-200">
            ...
            <div className="text-lg">{totalCount}</div>
          </div>

          <div className="bg-white p-2 rounded-lg border border-gray-200">
            <div className="text-xs text-gray-500 mb-0.5">Done</div>
            <div className="text-lg text-green-600">{completedCount}</div>
          </div>

          <div className="bg-white p-2 rounded-lg border border-gray-200">
            <div className="text-xs text-gray-500 mb-0.5">Rate</div>
            <div className="text-lg text-blue-600">{completionRate}%</div>
          </div>
        </div>
                ...
                <div className="p-2.5">
                  <div className="flex items-start gap-2.5 mb-2">
                    ...
                    >
                      {todo.isCompleted ? (
                        ...
                        <Circle className="w-6 h-6 text-gray-400 active:text-blue-600 transition-colors" />
                      )}
                    </button>

                    {/* Content */}
                    <div className="flex-1 min-w-0">
                      <h3 className={`text-sm mb-0.5 ${todo.isCompleted ? 'line-through text-gray-500' : ''}`}>
                        {todo.title}
                      </h3>
                      <p className={`text-xs text-gray-500 mb-2 ${todo.isCompleted ? 'line-through' : ''}`}>
                        {todo.description}
                      </p>

                      <div className="flex items-center gap-2 flex-wrap">
                        <span className={`px-2 py-0.5 rounded text-xs border ${priorityColors[todo.priority]}`}>
                  ...
                  <div className="pt-2 border-t border-gray-200">
                    ...
                    <textarea
                      ...
                      className={`w-full p-2 border rounded text-xs focus:ring-1 focus:ring-blue-500 focus:border-blue-500 ${
                    ...
                    {hasUnsavedChanges && (
                      <button
                        ...
                        className="mt-1 px-3 py-1 bg-blue-600 text-white rounded text-xs active:bg-blue-700 transition-colors flex items-center gap-1"
                      >
        ...
        <div className="bg-white p-3 rounded-lg border border-gray-200">
          <h3 className="text-sm mb-2">7-Day Completion Trend</h3>
